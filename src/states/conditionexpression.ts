...
export default class ConditionExpression extends State {
   ...
   * Using Free Marker expressions with Oracle BotML (and this JavaScript library) will require
   * reading through quite a bit of documentation....
   *
   ...
   * @see https://docs.oracle.com/en/cloud/paas/digital-assistant/use-chatbot/built-components-properties-transitions-and-usage.html#GUID-A41D3E53-76DE-49DB-B686-F0EBDE538349
   */
  private defineExpression(source: Variable|string,
    value: string|boolean|number, expression: string): this {
    // eslint-disable-next-line no-param-reassign
    value = typeof value === 'string' ? `"${value}"` : value;
    this.setValue(true);
    return this.setSourceExpression(
      `<#if ${(source as Variable).getDisplayName() || source} ${expression} ${value}
      >true<#else>false</#if>`,
    );
  }
  /**
