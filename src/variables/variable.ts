...
export default abstract class Variable {
  ...
  private generateUID(): string {
    return `Variable${Variable.count += 1}`;
  }
  ...
  /**
   * @param {Entity} entity
   * @param {number} position The position in the matched array,
   * starting from the most recent value matched.
   ...
   * should result in this being generated in the
   * end-YAML: "${iResult.value.entityMatches['PizzaSize'][0]}"
   */
